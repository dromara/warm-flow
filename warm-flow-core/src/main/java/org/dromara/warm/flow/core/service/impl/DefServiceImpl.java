...
public class DefServiceImpl extends WarmServiceImpl<FlowDefinitionDao<Definition>, Definition> implements DefService {
    ...
    @Override
    public void hisToDefJson(Long instanceId) {
        List<Instance> instanceList = new ArrayList<>();
        if (instanceId == null) {
            instanceList = FlowEngine.insService().list(FlowEngine.newIns());
        } else {
            instanceList.add(FlowEngine.insService().getById(instanceId));
        }
        FlowChartChain flowChartChain = new FlowChartChain();
        for (Instance instance : instanceList) {
            DefJson defJson = FlowEngine.defService().queryDesign(instance.getDefinitionId());
            Map<String, Color> colorMap = new HashMap<>();
            Map<String, ChartStatus> nodMap = addNodeChart(colorMap, instance, flowChartChain);
            Map<String, ChartStatus> skipMap = addSkipChart(colorMap, instance, flowChartChain);
            defJson.getNodeList().forEach(node -> {
                node.setStatus(nodMap.get(node.getNodeCode()).getKey());
                node.getSkipList().forEach(skip -> skip.setStatus(skipMap.get(getSkipKey(skip)).getKey()));
            });
            instance.setDefJson(FlowEngine.jsonConvert.objToStr(defJson));
        }
        FlowEngine.insService().updateBatch(instanceList);
    }
     ...
     */
    private Map<String, ChartStatus> addSkipChart(Map<String, Color> colorMap, Instance instance, FlowChartChain flowChartChain) {
        List<Skip> skipList = FlowEngine.skipService().list(FlowEngine.newSkip().setDefinitionId(instance.getDefinitionId()));
        Map<String, ChartStatus> chartMap = new HashMap<>();
        for (Skip skip : skipList) {
            if (StringUtils.isNotEmpty(skip.getCoordinate())) {
                ...
                if (Color.PINK.equals(c)) {
                    chartMap.put(getSkipKey(skip), ChartStatus.TO_DO);
                } else if (Color.GREEN.equals(c)) {
                    chartMap.put(getSkipKey(skip), ChartStatus.DONE);
                } else {
                    chartMap.put(getSkipKey(skip), ChartStatus.NOT_DONE);
                }
            }
        }
        return chartMap;
     ...
     * @param flowChartChain 流程图链
     */
    private Map<String, ChartStatus> addNodeChart(Map<String, Color> colorMap, Instance instance
            , FlowChartChain flowChartChain) {
        Map<String, ChartStatus> chartMap = new HashMap<>();
        List<Node> nodeList = FlowEngine.nodeService().list(FlowEngine.newNode().setDefinitionId(instance.getDefinitionId()));
        ...
        for (Node node : nodeList) {
            if (StringUtils.isNotEmpty(node.getCoordinate())) {
                ...
                if (Color.PINK.equals(c)) {
                    chartMap.put(node.getNodeCode(), ChartStatus.TO_DO);
                } else if (Color.GREEN.equals(c)) {
                    chartMap.put(node.getNodeCode(), ChartStatus.DONE);
                } else {
                    chartMap.put(node.getNodeCode(), ChartStatus.NOT_DONE);
                }
            }
        }
        return chartMap;
