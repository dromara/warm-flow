...
public class DefServiceImpl extends WarmServiceImpl<FlowDefinitionDao<Definition>, Definition> implements DefService {
    ...
    public boolean active(Long defId) {
        ...
        if (Objects.equals(definition.getActivityStatus(), ActivityStatus.ACTIVITY.getKey())) {
            return true;
        }
    ...
    public boolean unActive(Long defId) {
        Definition definition = getById(defId);
        if (Objects.equals(definition.getActivityStatus(),ActivityStatus.SUSPENDED.getKey())) {
            return true;
        }
