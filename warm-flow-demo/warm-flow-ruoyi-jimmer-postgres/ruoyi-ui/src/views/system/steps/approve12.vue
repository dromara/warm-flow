  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-if="open" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="110px" :disabled="disabled">
        ...
        <el-form-item label="出具发票" prop="issueInvoice">
          <div v-if="infoVo && Object.keys(infoVo).length">{{ form.issueInvoice }}</div>
          <el-input v-else :disabled="true" v-model="form.issueInvoice" placeholder="请输入出具发票" />
        </el-form-item>
        <el-form-item label="做好记录" prop="maintainRecords">
          <div v-if="infoVo && Object.keys(infoVo).length">{{ form.maintainRecords }}</div>
          <el-input v-else :disabled="true" v-model="form.maintainRecords" type="textarea" placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="入库" prop="warehousing">
          <div v-if="infoVo && Object.keys(infoVo).length">{{ form.warehousing }}</div>
          <el-input v-else :disabled="true" v-model="form.warehousing" placeholder="请输入入库" />
        </el-form-item>
        <el-form-item label="接收物品" prop="receiveItems">
          <div v-if="infoVo && Object.keys(infoVo).length">{{ form.receiveItems }}</div>
          <el-input v-else v-model="form.receiveItems" placeholder="请输入接收物品" />
        </el-form-item>
