-- public.flow_definition definition
...
CREATE TABLE flow_definition (
	...
	CONSTRAINT flow_code_version UNIQUE (flow_code, version),
...
COMMENT ON COLUMN public.flow_definition.from_custom IS '审批表单是否自定义（Y是 N否）';
COMMENT ON COLUMN public.flow_definition.from_path IS '审批表单路径';
...
-- public.flow_his_task definition

-- Drop table

-- DROP TABLE flow_his_task;
...
CREATE TABLE flow_his_task (
	...
	action_type int2 NOT NULL, -- 历史任务动作类型（0审批 1转办 2会签 3票签 4委派）
...
COMMENT ON TABLE public.flow_his_task IS '历史任务记录表';
...
COMMENT ON COLUMN public.flow_his_task.action_type IS '历史任务动作类型（0审批 1转办 2会签 3票签 4委派）';
...
COMMENT ON COLUMN public.flow_his_task.collaborator IS '协作人(只有转办、会签、票签、委派)';
COMMENT ON COLUMN public.flow_his_task.flow_status IS '流程状态（0待提交 1审批中 2 审批通过 8已完成 9已退回 10失效）';
COMMENT ON COLUMN public.flow_his_task.message IS '审批意见';
COMMENT ON COLUMN public.flow_his_task.create_time IS '创建时间';
COMMENT ON COLUMN public.flow_his_task.update_time IS '更新时间';
COMMENT ON COLUMN public.flow_his_task.del_flag IS '删除标志';
COMMENT ON COLUMN public.flow_his_task.tenant_id IS '租户id';


-- public.flow_instance definition

-- Drop table

-- DROP TABLE flow_instance;
...
COMMENT ON COLUMN public.flow_instance.node_type IS '结点类型（0开始节点 1中间节点 2结束节点 3互斥网关 4并行网关）';
...
COMMENT ON COLUMN public.flow_instance.flow_status IS '流程状态（0待提交 1审批中 2 审批通过 8已完成 9已退回 10失效）';
...
COMMENT ON TABLE public.flow_node IS '流程结点表';

-- Column comments

COMMENT ON COLUMN public.flow_node.id IS '主键id';
COMMENT ON COLUMN public.flow_node.node_type IS '节点类型（0开始节点 1中间节点 2结束结点 3互斥网关 4并行网关）';
...
COMMENT ON COLUMN public.flow_node.skip_any_node IS '是否可以退回任意节点（Y是 N否）';
...
-- public.flow_skip definition

-- Drop table

-- DROP TABLE flow_skip;
...
COMMENT ON TABLE public.flow_skip IS '结点跳转关联表';
...
COMMENT ON TABLE public.flow_task IS '待办任务表';
...
COMMENT ON COLUMN public.flow_task.flow_status IS '流程状态（0待提交 1审批中 2 审批通过 8已完成 9已退回 10失效）';
...
COMMENT ON COLUMN public.flow_user."type" IS '人员类型（1代办任务的审批人权限 2代办任务的转办人权限 3流程实例的抄送人权限 4待办任务的委托人权限）';
COMMENT ON COLUMN public.flow_user.processed_by IS '权限人';
COMMENT ON COLUMN public.flow_user.associated IS '关联表id';
