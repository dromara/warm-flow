...
comment on column FLOW_HIS_TASK.PERMISSION_FLAG is '权限标识 (权限类型:权限标识, 可以多个, 如role:1,role:2)'
/
...
comment on column FLOW_NODE.PERMISSION_FLAG is '权限标识 (权限类型:权限标识, 可以多个, 如role:1,role:2)'
/
...
comment on column FLOW_TASK.PERMISSION_FLAG is '权限标识 (权限类型:权限标识, 可以多个, 如role:1,role:2)'
/

comment on column FLOW_TASK.FLOW_STATUS is '流程状态 (0待提交 1审批中 2审批通过 8已完成 9已退回 10失效)'
/

comment on column FLOW_TASK.APPROVER is '审批者'
/

comment on column FLOW_TASK.ASSIGNEE is '转办人'
/
