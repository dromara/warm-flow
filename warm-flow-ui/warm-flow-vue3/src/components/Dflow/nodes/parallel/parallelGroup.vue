<template>
    <div style="position: relative;">
        <el-button v-if="!$attrs.readonly" dark round  size="small"  class="flow-serial-group-add-btn" @click="flowNodeSwitchAdd">添加并行</el-button>
        <div class="flow-serial-group">
            
            <div  class="flow-serial-group-line"  v-for="(item) in childNodes" :key="item.nodeId" >
                <div  class="flow-serial-group-wrap">
                    
                        <FlowRender 
                            v-bind="{...$attrs, ...subItem}" 
                            v-model="item[index]"
                            v-for="(subItem,index) in item"   
                            :key="subItem.nodeId" 
                            :backgroundImage="backgroundImage" ></FlowRender>
                        <!-- <FlowRender :nodeData="subItem"  v-for="subItem in item"  :key="subItem.nodeId" :readonly="readonly" ></FlowRender> -->
                </div>
            </div>
        </div>
        </div>
...
<script setup>
    ...
    const flowNodeSwitchAdd = () => {
        ...
        childNodes.splice(childNodes.length, 0 ,[{
                        ...
                        type: 'between',
                        properties:{},
                        nodeName:'审批人' + (childNodes.length + 1) ,
                        value: '',
