...
- [~] **④ god SFC 拆分（进行中）**：FlowDesigner → 容器编排 + `Header`/`Toolbar`/`StepNav`/`CanvasHost` 子组件 + 逻辑入 composables（`useFlowSave`/`useFlowValidate`…，沿用已有 `useLogicFlowCanvas`）；selectUser → `UserPicker` 容器 + 列表/树/已选 子组件 + `useUserPicker`。目标单 SFC < 400 行。
        ...
        - [x] **工具栏 `FlowDesignerToolbar.vue`（已落地·运行时验证）**：把画布工具栏（缩放/适应/放大 + 撤销/重做/清空 + 下载图/JSON + 只读态保存 + `toolbar-extra` 插槽）抽为子组件，状态仍由容器持有（props 入 / 事件出，父接既有 `zoomViewport`/`undoOrRedo`/… 方法），样式沿用 FlowDesigner 全局类、零迁移。
        - [x] **selectUser 逻辑层 `useUserPicker.ts`（已落地·运行时验证）**：把 selectUser 全部 state / 数据请求 / 勾选与已选维护抽到 `src/composables/useUserPicker.ts`（去 `getCurrentInstance` proxy，改用模板 ref `groupTreeRef`/`queryRef` + `nextTick`），SFC 仅留模板 + props/emit + 编排（562→331 行，达成 ④ < 400 行阶段目标）；**模板/样式零改动**（`git diff` 证 hunk 全在 `<script setup>` 内）。
              验证：`build:lib` 四产物 exit 0、**CSS 151.41KB 字节不变**（零样式变更）、es.js 341.81→343.70KB（composable `return`/解构合理增量）、改动文件 0 lint；EP demo `pnpm build` exit 0；**EP demo + Playwright 实跑**人员选择弹窗：tab 切换重查（角色 4 条↔用户 6 条）、树节点筛选、勾选→已选栏 + mini-tag + 半选表头、`parseTime` 日期格式化、点「确定」回填 between 办理人表（role:1/role:2）+ 画布节点办理人显示更新 + 弹窗关闭，控制台 0 报错 0 警告、截图与基线一致。
        - [x] **selectUser 视觉子组件之一 `UserPickerTree.vue`（已落地·运行时验证）**：把左侧组织架构树（卡片 + 可折叠头 + 部门搜索 + el-tree）抽为子组件。因 selectUser 样式为 **scoped**（子组件内部 DOM 命不中父 scoped 类），子组件**自带样式** `userPickerTree.scoped.scss`（从 selectUser 迁出 `.section-card`(复制)/`.tree-card`/`.tree-slide`/移动端 `.tree-card`，移动端块去掉 `.user-select-wrapper` 外层嵌套以适配子组件根）；`groupOptions` 入、`groupName`/`treeCollapsed` 走 v-model、`node-click` 事件出，el-tree 的 `filter`/`setCurrentKey` 经 `defineExpose` 转发给父（`useUserPicker` 的 `groupTreeRef` 改指向子组件，调用方式不变）；`filterNode` 随树下沉。selectUser.vue 模板树块（~40 行）外移、`selectUser.scoped.scss` 减 ~156 行。
              验证：`build:lib` 四产物 exit 0、CSS 151.41→151.77KB（+`.section-card` 复制 + 新 scoped data-v）、es.js 343.70→352.07KB（新 SFC 编译开销；**es.js 内 CSS 内联 0、树 SVG path 仅 1 次=无重复**）、改动文件 0 lint；EP demo `pnpm build` exit 0；**EP demo + Playwright 实跑**：树渲染与基线一致、搜索「后端」经暴露 `filter` 过滤到 总公司→研发中心→后端组、节点点击 `handleNodeClick` 重查无错、「重置」经暴露 `setCurrentKey` + 清空 groupName 还原全树、端到端选中→确定回填 between 办理人表(role:1) + 节点显示更新 + 弹窗关闭，控制台 0 报错 0 警告、截图像素一致。
        - [x] **selectUser 视觉子组件之二 `UserPickerSelectedBar.vue`（已落地·运行时验证）**：把「已选提示栏」（已选计数 + mini-tag 列表 + 超 8 个的 `+N 更多` tooltip + 清空）抽为子组件。最干净的一个——**无 ref 透传**：`selected`（即 checkedItemList）入、`close(storageId)`/`clear` 事件出，`visibleTags`/`visibleTagsCount` 随之下沉到子组件内部计算（从 `useUserPicker` 移除）。子组件自带 `userPickerSelectedBar.scoped.scss`（迁出 `.selected-inline-bar` + `.selected-all-tooltip` + 移动端块）。selectUser.vue 由 290→**256 行**。
              验证：`build:lib` 四产物 exit 0、CSS 151.77→151.77KB（≈持平）、es.js 352.07→351.96KB（略降；sanity: CSS 内联 0、勾选 checkmark SVG path 2 次=已选栏 bar-icon + 移动端卡片各 1，非重复）、改动文件 0 lint；EP demo `pnpm build` exit 0；**EP demo + Playwright 实跑**：勾选 2 行→已选栏渲染（已选择 2 项 + 两 mini-tag + 半选表头）、点标签 × 经 `close`→`handleClose` 减为 1 项且对应行取消勾选、「清空」经 `clear`→`clearAllSelected` 整栏消失、端到端选中→确定回填 between 办理人表（role:1/role:2）+ 弹窗关闭，控制台 0 报错 0 警告、截图像素一致。
        - [x] **selectUser 视觉子组件之三 `UserPickerSearch.vue`（已落地·运行时验证）**：把「移动端筛选切换行 + 查询表单卡片（权限编码/名称/创建时间 + 查询/重置/确定）」抽为子组件（fragment 双根）。`queryParams`（共享响应式对象，handlerCode/handlerName 直接写入）入、`dateRange` 走 v-model、`query`/`reset`/`confirm` 事件出，`searchCollapsed`/`showSearch` 内部化（从 `useUserPicker` 移除），el-form 的 `resetFields` 经 `defineExpose` 转发给父（`queryRef` 指向子组件）。子组件自带 `userPickerSearch.scoped.scss`（迁出 `.search-card`/`.search-card-body`/`.btn-action`/`.btn-confirm` + 复制 `.section-card` + `@keyframes` + 从交错的移动端 `@media` 精确剥离搜索规则，保留列表/布局规则在 selectUser）。selectUser.vue 256→**214 行**。
              验证：`build:lib` 四产物 exit 0、CSS 151.77→151.98KB（+`.section-card` 复制/新 data-v）、es.js 351.96→352.11KB、改动文件 0 lint；EP demo `pnpm build` exit 0；**EP demo + Playwright 实跑**：搜索表单渲染与基线一致、填编码/名称→查询经共享 queryParams 写入触发 getList 无错、「重置」经暴露 resetFields 清空两输入、移动端筛选行桌面正确隐藏、树/已选栏/表格 + 端到端选中→确定回填 between 表(role:1)+弹窗关闭 仍正常，控制台 0 报错 0 警告、截图像素一致。
        - [x] **selectUser 拆分收口（balanced，遵循「不过度拆分」）**：已抽 `useUserPicker`（逻辑层）+ `UserPickerTree`/`UserPickerSelectedBar`/`UserPickerSearch` 三个内聚展示子组件，`selectUser.vue` 562→**214 行**、逻辑/视图/样式分层、零观感、每步运行时验证。**列表（PC 表格 + 移动卡片 + 分页 + 全选/勾选）刻意保留在容器内联**：它与选择状态（`checkedItemList`/`checkAllInfo`/`tableList.isChecked`，均在 `useUserPicker`）强耦合，单独抽出需 ~12 个 props/events 的「超宽接口」，属「为拆而拆」；列表本就是该选择器的核心内容，由容器直接持有更清晰。故 selectUser 拆分停在此（不为凑 < 150 行而过度拆分）。
        - [ ] **剩余（FlowDesigner 侧，按需、低优先）**：`CanvasHost`（画布容器 + PropertySetting 编排 + 全插槽透传）同属「超宽接口」风险、价值低，暂不做；FlowDesigner 逻辑分层（`useFlowSave`/`useFlowValidate`，同 `useUserPicker` 范式）可作后续 ⑥ 的一部分按需推进。
